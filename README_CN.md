...
支持 `gpt-image-2.5-flare`、`gpt-image-2.5-sunburst` 及其 `2026-09-08` 日期快照，可通过 `/v1/images/generations`、`/v1/images/edits` 调用。`quality` 支持 `xhigh`、`max`、`auto`，图片 usage 明细保持透传。

OAuth / Setup Token 图片请求使用 Responses 主控模型调用 `image_generation` 工具，默认主控为 `gpt-5.6-luna`。可设置 `SUB2API_IMAGES_MAIN_MODEL` 切换为账号支持的文本模型；该配置不会替换所选图片模型。

已有显式账号映射或分组白名单需要加入新模型；升级不会自动扩大管理员设置的模型权限。新模型内置文本输入、图片输入和图片输出 token 费率，远端价格表尚未更新时仍可正确计费。
