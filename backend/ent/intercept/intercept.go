...
type FileObjectFunc func(context.Context, *ent.FileObjectQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f FileObjectFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.FileObjectQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.FileObjectQuery", q)
}

// The TraverseFileObject type is an adapter to allow the use of ordinary function as Traverser.
type TraverseFileObject func(context.Context, *ent.FileObjectQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseFileObject) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseFileObject) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.FileObjectQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.FileObjectQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.FileObjectQuery:
		return &query[*ent.FileObjectQuery, predicate.FileObject, fileobject.OrderOption]{typ: ent.TypeFileObject, tq: q}, nil
	case *ent.GroupQuery:
