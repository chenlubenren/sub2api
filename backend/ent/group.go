...
type Group struct {
	...
	// 内部夜间计费规则开关，不对用户端暴露
	NightRateEnabled bool `json:"night_rate_enabled,omitempty"`
	// 内部夜间计费开始时间 HH:MM
...
func (*Group) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case group.FieldName, group.FieldDescription, group.FieldPeakStart, group.FieldPeakEnd, group.FieldNightStart, group.FieldNightEnd, group.FieldStatus, group.FieldDuplicateOperationID, group.FieldPlatform, group.FieldSubscriptionType, group.FieldDefaultMappedModel, group.FieldMaxReasoningEffort, group.FieldMaxReasoningEffortOverLimit:
...
func (_m *Group) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case group.FieldNightRateEnabled:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field night_rate_enabled", values[i])
			} else if value.Valid {
				_m.NightRateEnabled = value.Bool
			}
		case group.FieldNightStart:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field night_start", values[i])
			} else if value.Valid {
				_m.NightStart = value.String
			}
		case group.FieldNightEnd:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field night_end", values[i])
			} else if value.Valid {
				_m.NightEnd = value.String
			}
		case group.FieldNightRateMultiplier:
			if value, ok := values[i].(*sql.NullFloat64); !ok {
				return fmt.Errorf("unexpected type %T for field night_rate_multiplier", values[i])
			} else if value.Valid {
				_m.NightRateMultiplier = value.Float64
			}
...
func (_m *Group) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("night_rate_enabled=")
	builder.WriteString(fmt.Sprintf("%v", _m.NightRateEnabled))
	builder.WriteString(", ")
	builder.WriteString("night_start=")
	builder.WriteString(_m.NightStart)
	builder.WriteString(", ")
	builder.WriteString("night_end=")
	builder.WriteString(_m.NightEnd)
	builder.WriteString(", ")
	builder.WriteString("night_rate_multiplier=")
	builder.WriteString(fmt.Sprintf("%v", _m.NightRateMultiplier))
	builder.WriteString(", ")
