...
func (i *PluginPackageInstaller) Install(ctx context.Context, reader io.Reader, installedBy *int64) (*PluginInstallation, error) {
	...
	// Windows keeps the ZIP file open through the ReadCloser until it is
	// explicitly closed. Close it before renaming the uploaded artifact;
	// otherwise os.Rename fails with "file is being used by another process".
